...
int main(int argc, char **argv){
  ...
  #pragma omp parallel for default(none) private(fila,columna) shared(matriz1,num_filas,num_columnas,chunk) schedule(dynamic,chunk)
  for(fila=0; fila<num_filas; fila++){
  ...
  #pragma omp parallel for default(none) private(fila,columna) shared(matriz2,num_filas,num_columnas,chunk) schedule(dynamic,chunk)
  for(fila=0; fila<num_filas; fila++){
    for(columna=0; columna<num_columnas; columna++){
  ...
  #pragma omp parallel for default(none) private(fila,columna,j) shared(matriz1,matriz2,num_filas,num_columnas,resultado,chunk) schedule(dynamic,chunk)
  for (fila = 0 ; fila < num_filas ; fila++ ){
     for (columna = 0 ; columna < num_columnas; columna++ ){
        int producto = 0 ;
